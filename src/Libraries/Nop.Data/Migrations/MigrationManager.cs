...
namespace Nop.Data.Migrations
{
    ...
    public partial class MigrationManager : IMigrationManager
    {
        ...
        /// <param name="isSchemaMigration">Indicate whether to found only schema migration; if set to true will by returns only that migrations which has the <see cref="MigrationConfig"/>.IsSchemaMigration property set to true</param>
            ...
            MigrationProcessType migrationProcessType = MigrationProcessType.NoMatter, bool isApplied = false, bool isSchemaMigration = false)
        {
            ...
            var migrations = _filteringMigrationSource
                .GetMigrations(t =>
                {
                    ...
                    if (isSchemaMigration && !migrationAttribute.MigrationConfig.IsSchemaMigration)
                        return false;
        ...
        /// <param name="assembly">Assembly to find migrations</param>
        /// <param name="migrationProcessType">Type of migration process</param>
        public virtual void ApplyUpSchemaMigrations(Assembly assembly, MigrationProcessType migrationProcessType = MigrationProcessType.Installation)
        {
            if (assembly is null)
                throw new ArgumentNullException(nameof(assembly));

            foreach (var migrationInfo in GetMigrations(assembly, false, migrationProcessType, isSchemaMigration: true).OrderBy(migration => migration.Version))
